...
export class CharacterService {
  ...
  searchCharacters(query = '', page = 200):Observable<Character[] | TrackHttpError> {
    const filter = `${environment.baseUrlAPI}/alimentosVentas/obtenerAlimentosVenta`;
    return this.http.get<Character[]>(filter)
    .pipe(catchError((err) => this.handleHttpError(err)));
  }

  getDetails(id: number):Observable<Character[] | TrackHttpError> {
    return this.http.get<Character[]>(`${environment.baseUrlAPI}/alimentosVentas/obtenerAlimentosCategoria/${id}`)
    .pipe(catchError((err) => this.handleHttpError(err)));
  }
